...
* **Thread**: Contains which worker thread was handling this logging process.
...
* **Database**: Store logs for querying and analysis (TODO)
...
* Configure overload handling policies (TODO)

Configuration must be simple, readable and modifiable at runtime without restarting the application.
...
* Configurable timestamp formats (TODO)
* Custom level representations

## 8. Correlation Context Management (USP #1)

The framework must support **first-class correlation context**:

* Correlation context can be explicitly set by the application
* Correlation context is automatically propagated across threads
* Correlation context is included in every log message

## 9. Overload Protection & Log Drop Strategy (USP #2) (TODO)
...
## 10. Logging Failure Visibility & Health Monitoring (USP #3) (TODO)
